    ...
    st.markdown(f"""
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <div class="weather-pill">
                🌦 Hà Nội · {weather_ctx['temp']}°C · {weather_ctx['description'].split(',')[1].split('.')[0].strip() if ',' in weather_ctx['description'] else 'Dễ chịu'}
            </div>
        </div>
...
def render_dish_card(dish: Dish, current_mood: str):
    ...
    # Process local fallback base64 if needed
    ...
    chips_html = f"""
        <span class="chip">💰 {dish.price_min//1000}K - {dish.price_max//1000}K</span>
        <span class="chip">🌶️ Cấp độ {dish.spice_level}</span>
        <span class="chip">⭐ {dish.popularity_score}/10</span>
    """
